...
async function takeSeleniumScreenshots() {
  ...
  try {
    // Screenshot 1: Naver.com
    console.log('📸 Taking screenshot of naver.com with Selenium...');
    await chrome.goto('https://www.naver.com');
    ...
    await chrome.saveScreenshot(path1);
    ...
    try {
      ...
      console.log('🔧 Selenium browser already closed');
    }
  }
}
...
async function main() {
  ...
  try {
    ...
    // List all files
    const files = fs.readdirSync(downloadsDir).filter(f => f.includes('playwright') || f.includes('selenium'));
    console.log('\n📋 Generated files:');
    files.forEach(file => {
